              ...
              <?php if(count($products_list) > 0 ) {?>  
  	        		<?php $i = 1; foreach ($products_list as $key) :?>
  		        		<tr id="Row<?= $key['ProductsID']?>">
                            ...
                            <?php
                            foreach ($sections as $section) {
                                $section_ids = array_map(function ($pivot) {
                                    return $pivot['section_id'];
                                }, $key['sections']);

                                echo '<td align="center" style="width:60px;text-align:center;">';
                                echo '<input type="checkbox" value="'.$section['id'].'" id="cbIsChecked'. $key['ProductsID'].'"';

                                if (in_array($section['id'], $section_ids))
                                    echo 'checked="checked"';
                                echo 'onclick="UpdateSectionPivot('.$key['ProductsID'].');"/>';
                                echo '</td>';
                            }
                            ?>
...
<script type="text/javascript">
 //<![CDATA[
  ...
  function UpdateSectionPivot(product_id) {
    var IsChecked;
    if($("#cbIsChecked"+product_id).attr('checked')){
      IsChecked = 1;  
    }else {
      IsChecked = 0;
    }
    const section_id = $("#cbIsChecked"+product_id).val();
    $.ajax( {
          type: "POST",
          url: "<?= base_url()?>ajaxhandle/admin_products_ajaxhandler/UpdateSectionPivot",
          data: { product_id, IsChecked, section_id},
          dataType: "json",
          success: function(data) {
            if(data == 0) {
              notice('Đã có lỗi! Thực hiện lại sau.');
            }else if(data == 1) {
              notice("Đã cập nhật trạng thái section cho sản phẩm.");
            }else {
              notice(data.msg);
            }
          },
